...
/**
 * Install the page-side bridge handler in the current window. Idempotent.
 * Used by the content-script entry below (host pages) and directly by the
 * onboarding entrypoint (extension pages, where content scripts can't run).
 */
  ...
  function setSelectValue(el: Element, label: string): boolean {
    ...
    const match =
      ...
      state.options.find((o: any) => {
        ...
        return l
          ? new RegExp('\\b' + normalized.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b').test(l) ||
